...
import { runSetup, connectTelegramUser, isInstalled, getGatewayStatus } from "./configure.js";
...
export async function startSetupServer() {
  ...
  app.get("/status", (_req, res) => {
    ...
    const gateway = installed ? getGatewayStatus() : "stopped";
    res.json({ installed, gateway });
  });

  // Start gateway
  app.post("/start-gateway", async (_req, res) => {
    try {
      execSync("openclaw gateway start --port 28789", { stdio: "inherit", timeout: 15000 });
  ...
  // Stop gateway
  app.post("/stop-gateway", async (_req, res) => {
    try {
      execSync("openclaw gateway stop", { stdio: "inherit", timeout: 15000 });
