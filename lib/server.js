...
import { getAllProfiles, MODEL_CATALOG, runSetupStream, connectTelegramUser, startGateway, stopGateway, restartGateway, deleteProfile } from "./configure.js";
...
export async function startSetupServer() {
  ...
  app.get("/", async (_req, res) => {
    ...
    const initScript = "<script>window.__STATE__=" + JSON.stringify({ profiles, models: MODEL_CATALOG }) + "</script>";
  ...
  app.get("/setup-stream", (req, res) => {
    ...
    runSetupStream(profile, apiKey, modelId, channel, channelCreds, sendProgress)
